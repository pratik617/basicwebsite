                  ...
                  <div class="col-sm-6">
                      ...
                      <div class="calculator-iteam">
                          ...
                          <div class="form-group">
                              <!-- User Name -->
                              <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ (session()->get('s_name'))?session()->get('s_name'):old('name') }}" placeholder="Name" data-validation="required" data-validation-error-msg-required="Please enter your name">
                              @if ($errors->has('name'))
                          ...
                          <div class="form-group">
                            ...
                            <input id="email" type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ (session()->get('s_email'))?session()->get('s_email'):old('email') }}" placeholder="Email" data-validation="required|email" data-validation-error-msg-required="Please enter your email address" data-validation-error-msg-email="Please enter correct email address">
                            @if ($errors->has('email'))
                                <span class="invalid-feedback" role="alert">
  ...
  //$(document).ready(function() {
    ...
    function initMap() {
      ...
      window.intlTelInput(input, {
        ...
        geoIpLookup: function(callback) {
          ...
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
              ...
              var latlng = {lat: parseFloat(customer_pos.lat), lng: parseFloat(customer_pos.lng)};
              geocoder.geocode({'location': latlng}, function(results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    if (results[0]) {
                        var country_code = getCountry(results);
        ...
        utilsScript: "js/customer/utils.js?1551697588835" // just for formatting/placeholders etc
      });
